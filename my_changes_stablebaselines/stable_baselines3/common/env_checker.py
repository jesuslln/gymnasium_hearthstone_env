...
def _check_unsupported_spaces(env: gymnasium.Env, observation_space: spaces.Space, action_space: spaces.Space) -> None:
...
def _check_nan(env: gymnasium.Env) -> None:
...
def _is_goal_env(env: gymnasium.Env) -> bool:
    """
    Check if the env uses the convention for goal-conditioned envs (previously, the gym.GoalEnv interface)
    """
    if isinstance(env, gymnasium.Wrapper):  # We need to unwrap the env since gym.Wrapper has the compute_reward method
...
def _check_returned_values(env: gymnasium.Env, observation_space: spaces.Space, action_space: spaces.Space) -> None:
    """
...
def _check_spaces(env: gymnasium.Env) -> None:
    """
...
def _check_render(env: gymnasium.Env, warn: bool = True, headless: bool = False) -> None:  # pragma: no cover
    """
...
def check_env(env: gymnasium.Env, warn: bool = True, skip_render_check: bool = True) -> None:
    """
