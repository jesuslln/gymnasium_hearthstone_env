...
def test_eval_friendly_error():
    # tests that eval callback does not crash when given a vector
    train_env = VecNormalize(DummyVecEnv([lambda: gymnasium.make("CartPole-v1")]))
    eval_env = DummyVecEnv([lambda: gymnasium.make("CartPole-v1")])
    ...
    wrong_eval_env = gymnasium.make("CartPole-v1")
    eval_callback = EvalCallback(
        wrong_eval_env,
        eval_freq=100,
        warn=False,
...
def test_checkpoint_additional_info(tmp_path):
    ...
    dummy_vec_env = DummyVecEnv([lambda: gymnasium.make("CartPole-v1")])
