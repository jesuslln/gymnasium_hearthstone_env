...
class DummyMixedDictEnv(gymnasium.Env):
    """
...
def make_env():
    return Monitor(gymnasium.make(ENV_ID))
...
def _make_warmstart_cliffwalking(**kwargs):
    ...
    return _make_warmstart(lambda: gymnasium.make("CliffWalking-v0"), **kwargs)


def _make_warmstart_cartpole():
    """Warm-start VecNormalize by stepping through CartPole"""
    return _make_warmstart(lambda: gymnasium.make("CartPole-v1"))
