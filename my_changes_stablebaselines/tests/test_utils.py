...
@pytest.mark.parametrize("env_id", ["CartPole-v1", lambda: gymnasium.make("CartPole-v1")])
@pytest.mark.parametrize("n_envs", [1, 2])
@pytest.mark.parametrize("vec_env_cls", [None, SubprocVecEnv])
@pytest.mark.parametrize("wrapper_class", [None, gymnasium.wrappers.TimeLimit])
...
class ZeroRewardWrapper(gymnasium.RewardWrapper):
    def reward(self, reward):
        return reward * 0


class AlwaysDoneWrapper(gymnasium.Wrapper):
...
def test_evaluate_policy_monitors(vec_env_class):
    ...
    def make_eval_env(with_monitor, wrapper_class=gymnasium.Wrapper):
        ...
        if vec_env_class is None:
            ...
            if with_monitor:
                env = vec_env_class([lambda: wrapper_class(Monitor(gymnasium.make(env_id)))] * n_envs)
            else:
                env = vec_env_class([lambda: wrapper_class(gymnasium.make(env_id))] * n_envs)
        return env
