...
class DummyDictEnv(gymnasium.Env):
...
def test_consistency(model_class):
    ...
    dict_env = gymnasium.wrappers.TimeLimit(dict_env, 100)
    env = gymnasium.wrappers.FlattenObservation(dict_env)
    dict_env.seed(10)
    obs = dict_env.reset()
...
def test_dict_spaces(model_class, channel_last):
    ...
    env = gymnasium.wrappers.TimeLimit(env, 100)
...
def test_multiprocessing(model_class):
    ...
    def make_env():
        ...
        env = gymnasium.wrappers.TimeLimit(env, 50)
        return env
...
def test_vec_normalize(model_class):
    ...
    """
    env = DummyVecEnv([lambda: gymnasium.wrappers.TimeLimit(DummyDictEnv(use_discrete_actions=model_class == DQN), 100)])
